...
8. **部署后最小检查。** 核对中英文首页、14–17 与 18–25 工作台、SWEET 保存与历史、旧 guardian session 和直接 API 拒绝、Account 注销、青序花园与提醒、轻量反馈；直达聊天室 URL/API 与正式咨询 URL/API 仍不能开放。生产写入型检查须另行指定合成账号和明确授权，不能用真实学生数据代替。
...
本机隔离证据：55 份迁移升级、迁移前 guardian 旧 session 失权、历史关系保留、定向修复 dry-run 回滚、历史关系下的 guardian 与学生注销、青序花园和反馈 API、默认关闭入口均已通过；定向 Playwright 63 passed、0 failed、3 个重复视口 skipped。以上仅支持**进入 MVP 正式部署准备**，不替代正式操作授权及仍开放的人工/学校条件。
