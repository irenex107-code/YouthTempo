...
下文保留的是修改前的完整审查快照。报告指出的主要代码级问题已进行小范围修复：Talk 保持关闭，Worry Time 与 Referral 使用固定规则；Check-in 与 Mood Journal 的模型职责进一步收敛为选择来源字段 ID，服务器严格校验后使用脱敏来源片段与固定模板组成最终小结，模型不能直接写入展示文本；普通模型请求绑定服务端登录和有效同意。发送前的常见标识移除已扩展到邮箱、电话、证件号、带标签姓名、学校、班级、地址、社交账号和链接；首批试点保持受限生成开启，仍须通过 HTTPS、provider 主机与模型白名单校验，并可用环境总开关暂停。“我觉得活着没有意义”“活着还有什么意思”及对应英文表达在任何模型调用前进入固定紧急路径，课程或作文讨论反例保持普通流程。中英文站内安全文案现优先连接家长、老师或其他可信任成年人，英文不固定中国大陆号码。

这些修改不等于无条件批准真实学生试点。代码级来源字段验证、provider/model allowlist 和 AI 总开关已经实现；日期模型快照、正式供应商治理、生成记录 provenance，以及法务、隐私、心理专业和学校签字作为后续条件继续完成。项目正式结论保持 **READY WITH CONDITIONS**。

## 同意政策增量（2026-08-28）

下文关于“14–17 岁必须绑定学校核验监护人确认”的内容是 2026-08-18 学校试点方案快照。2026-08-28 已批准当前无学校、老师或监护人参与的学生自主试用政策：仅接收 14 岁及以上学生；14–17 岁完成本人敏感信息与 AI 处理单独确认后，可使用 AI 小结并保存本人记录。同意依据明确记录为 `student_self_pilot`，不会伪造监护人确认或创建任何访问关系；正式学校试点必须发布新的同意版本并重新审核适用确认。该变更不关闭本报告中的 provider 治理、心理专业审核、危机承接或其他 `READY WITH CONDITIONS` 项。
