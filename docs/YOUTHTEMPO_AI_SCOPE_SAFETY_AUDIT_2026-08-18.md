...
下文保留的是修改前的审查证据和风险判断。审查后已按建议完成首轮技术收敛：Worry Time 与 Referral 均改为版本化固定规则；Check-in 与 Mood Journal 的模型输出进一步收敛为来源字段 ID 选择，服务器严格校验后以脱敏来源片段和固定模板组成小结；普通生成绑定服务端登录与有效同意，并增加扩展标识移除、生成总开关、provider 主机白名单、模型白名单和 OpenAI 日期快照校验；“我觉得活着没有意义”及对应中英文被动轻生表达已进入模型调用前的确定性安全路径。公开文案统一优先连接家长、老师或其他可信任成年人，英文紧急提示只使用当地紧急服务。

当前仍不能标记为无条件 `READY`：代码级严格来源选择、模型 allowlist/snapshot 和总开关已经实现，但正式环境获批配置证据、provider 保留/训练/处理地点/跨境评估、生成记录 provenance，以及法务、隐私、心理专业和学校签字仍是 `ROADMAP.md` 中的试点阻塞项。正式运营结论保持 **READY WITH CONDITIONS**，真实学生入组前必须关闭这些条件。
