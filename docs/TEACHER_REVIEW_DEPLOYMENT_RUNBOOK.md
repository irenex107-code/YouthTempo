...
2. 在**独立 E2E 项目**先按下面顺序执行迁移与 fixture，运行浏览器、直接 Data API、RLS、旧 session、Storage 和删除测试。当前本地数据库未启动成功，此门槛仍开放。
...
先处理此前未合并的 `20260912161714_allow_individual_professional_applicants.sql`（机构可选）及 `20260917*` 解忧室访问、值班基础和 `20260919065043_add_peer_space_room_state_controls.sql`，然后：
...
在独立项目核对所有迁移版本和前后 schema，执行 `supabase db lint`、RLS advisor、角色矩阵与真实 SQL。当前 Colima 可启动但本地 Docker 镜像拉取遇 DNS 阻断，**没有 Migration 执行或 RLS 通过证据**。生产 dry-run 仅做只读计数和 `BEGIN ... ROLLBACK` 事务预演，真实 `COMMIT` 要另行批准。迁移前保留数据库备份与应用镜像；DDL 的回滚必须由 DBA 评估依赖和数据保留，不能简单删表。历史关系修复在事务未提交前用 `ROLLBACK`；提交后不得自动恢复监护读取，应通过新批准的政策、同意和补偿迁移处理。应用异常按现有 `scripts/deploy-lighthouse.sh` 回退旧镜像，同时保持新开关关闭；若 RLS 安全迁移已生效，不能为了应用回退恢复家长访问。
