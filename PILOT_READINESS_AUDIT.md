...
| P0 | 模型可直接写最终小结，来源字段、provider 与模型版本未被严格约束 | 模型现只返回最多 2/3 个来源字段 ID；服务器拒绝未知、重复、超量或额外字段，并使用经脱敏的原文片段和固定模板组成最终小结。生成默认关闭，显式开启后仍须通过 HTTPS、provider 主机白名单、模型白名单及 OpenAI 日期快照校验。正式环境配置、供应商尽调、跨境评估与数据库 provenance 仍是开放条件 |
...
- 2026-08-18 AI 来源约束与配置收口增量：模型只选来源字段 ID、服务器严格校验与固定模板、扩展标识移除、生成总开关、provider/model allowlist、OpenAI 日期快照及 B–G 越界输出拒绝共 90 passed（desktop/mobile Chromium）；`pnpm typecheck` 与 `pnpm build` 通过，构建 63 routes。未调用真实 provider，供应商后台与正式环境配置仍需人工验收。
